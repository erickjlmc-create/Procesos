...
# NOTA DE OPTIMIZACION (ver metodologia.md / proyecto de backtesting):
# Los siguientes 6 valores fueron endurecidos tras comparar backtests reales
# antes/despues en 165-262 operaciones sobre 19 pares (Yahoo Finance, 15m):
#   - SRSI_OB/OS y WR_OB/OS mas estrictos -> Drawdown maximo bajo de -32.8% a
#     -22.0%, y el grado "prec" paso de Profit Factor 0.67 a 1.05.
#   - STC_BULL/BEAR mas estrictos -> mejora marginal pero consistente en
#     Profit Factor, PnL y Drawdown (11/19 pares a favor).
# Los valores originales quedan comentados a la derecha para referencia.
CFG = {
    "ST_PERIOD":10,"ST_FACTOR":3.0,"EMA_FAST":9,"EMA_SLOW":21,"MFI_PERIOD":14,
    "STC_CYCLE":10,"STC_FAST":23,"STC_SLOW":50,"STC_BULL":30,"STC_BEAR":70,  # antes: 25 / 75
    "SRSI_RSI":14,"SRSI_STOCH":14,"SRSI_K":3,"SRSI_D":3,"SRSI_OB":75,"SRSI_OS":25,  # antes: 85 / 15
    "WR_LEN":14,"WR_OB":-25,"WR_OS":-75,  # antes: -20 / -80
...
def analyze_pair(pair):
    ...
    # NOTA: adx_val/chop_val/atr_pct se calculan SOLO para mostrarlos como
    # referencia visual en las notificaciones (igual que en el indicador Pine,
    # donde son informativos y no determinan ninguna señal). Las variables
    # adx_ok/chop_ok/regime_ok/atr_ok que existian aqui antes NUNCA se usaban
    # para nada en la logica de senales (confirmado revisando bull_ctx/bear_ctx
    # y b_l/b_s) — se eliminaron por ser codigo muerto.
    adx_val=calc_adx(c15,CFG["ADX_PERIOD"])
    chop_val=calc_choppiness(c15,CFG["CHOP_LEN"])
    ...
    # ── Confirmación 1H para señales App / Ap / B ────────────────────
    # ANTES: solo B pedia esta confirmacion. Se extendio a App y Ap porque el
    # backtest mostro que B (menos pilares en 15m + confirmacion 1H) supera
    # consistentemente a App/Ap (mas pilares en 15m + SIN confirmacion 1H) en
    # Profit Factor. "prec" se deja sin este requisito porque ya se corrigio
    # via guardias/STC y no tiene el mismo problema estructural.
    h1_conf_l = h1_conf_s = False
    if app_l or ap_l or b_l or app_s or ap_s or b_s:
        try:
            ...
            if len(c1h) >= 30:
                ...
                h1_conf_l = st1h[-1]==1  and e9_1h > e21_1h
                h1_conf_s = st1h[-1]==-1 and e9_1h < e21_1h
        except Exception:
            h1_conf_l = h1_conf_s = False
    h1_confirmed = h1_conf_l or h1_conf_s  # se conserva el campo por compatibilidad con el resto del bot

    if   (app_l and h1_conf_l) or (app_s and h1_conf_s):  grade="App"
    elif (ap_l  and h1_conf_l) or (ap_s  and h1_conf_s):  grade="Ap"
    elif b_l   or b_s:      grade="B" if (h1_conf_l or h1_conf_s) else "none"
    elif prec_l or prec_s:  grade="prec"
    elif blk_l or blk_s:    grade="blocked"
    else:                    grade="none"

    if   (app_l and h1_conf_l) or (ap_l and h1_conf_l) or (b_l and h1_conf_l) or prec_l:   direction="long"
    elif (app_s and h1_conf_s) or (ap_s and h1_conf_s) or (b_s and h1_conf_s) or prec_s:   direction="short"
    ...
    for pair in PAIRS:
        ...
        if fd != "none" and grade == "none" and fd != prev_forming:
            ...
            notify(
                ...
                destino="radar",  # <-- antes iba al canal principal (default); generaba demasiado ruido
            )
